...
public class AsyncExecutorUtil<T> {
	...
	public List<Future<T>> doInvokeAllWithFixedThread(List<Callable<T>> taskList ,int threadNum,long timeout){
		ExecutorService executorService = Executors.newFixedThreadPool(threadNum);
		List<Future<T>> list = Lists.newArrayList();
		try {
			list = executorService.invokeAll(taskList , timeout , TimeUnit.MILLISECONDS);
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}finally{
